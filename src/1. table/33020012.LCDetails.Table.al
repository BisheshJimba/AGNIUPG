table 33020012 "LC Details"
{
    ...
    fields
    {
        field(1; "No."; Code[20])
        {
            ...
            begin
                IF "No." <> xRec."No." THEN BEGIN
                    LCSetup.GET;
                    NoSeriesMngt.TestManual(LCSetup."LC Details Nos.");
                    "No. Series" := '';
                END;
            end;
        }
            ...
            begin
                IF ("Transaction Type" = "Transaction Type"::Purchase) THEN BEGIN
                  ...
                  IF GblBankAcc.FIND('-') THEN BEGIN
                    "Issue Bank Name1" := GblBankAcc.Name;
                    MODIFY;
                  END;
                END ELSE IF ("Transaction Type" = "Transaction Type"::Sale) THEN BEGIN
                  ...
                  IF GblCustBankAcc.FIND('-') THEN BEGIN
                    "Issue Bank Name1" := GblCustBankAcc.Name;
        ...
        field(13;"Exchange Rate";Decimal)
        {
            ...
            trigger OnValidate()
            begin
                VALIDATE("LC\BG Value");
            end;
        }
        field(14;"LC\BG Value";Decimal)
        {
            Caption = 'LC\BG Value';
            ...
            begin
                IF "Currency Code" <> '' THEN BEGIN
                  ...
                  "LC Value (LCY)" := ROUND("LC\BG Value" * "Exchange Rate",Currency."Amount Rounding Precision");
                END ELSE
                  "LC Value (LCY)" := "LC\BG Value";
        ...
        field(29;"Issued To/Received From Name";Text[70])
        {
        }
        ...
        field(36;"Total LC Allotment";Decimal)
        {
            CalcFormula = Sum("LC Value Allotment"."Value (LCY)" WHERE (LC Code=FIELD(No.)));
            Editable = false;
            FieldClass = FlowField;
        }
        field(37;"Total LC Charge";Decimal)
        {
            CalcFormula = Sum("LC Value Entries"."Charge (LCY)" WHERE (LC Code=FIELD(No.),
                                                                       Type=FIELD(Allotment Type),
                                                                       Item Charge=CONST(No)));
            Editable = false;
            FieldClass = FlowField;
        }
        ...
        field(40;"Insured By";Code[50])
        {
            TableRelation = Vendor;
        }
        field(41;Incoterms;Option)
        {
            OptionCaption = ' ,C&F,CIF,CIP,FOB,CFR,C&I,CNI,CPT,EX-WORKS';
            OptionMembers = "  ","C&F",CIF,CIP,FOB,CFR,"C&I",CNI,CPT,"EX-WORKS";
        }
        field(42;Notes;Text[250])
        {
        }
        field(43;"Purchase LC Utilised";Decimal)
        {
            CalcFormula = Sum("G/L Entry".Amount WHERE (System LC No.=FIELD(No.),
                                                        G/L Account No.=FILTER(400000..499999),
                                                        Source Type=FILTER(Vendor)));
            Description = 'Vendors used (V0009|V01088|V01112|V01201|V01247|V0210|V0211|V0235|V0344|V0347|V0411|V0481|V0482|V0497|V0507|V0509|V0927)';
            Editable = false;
            FieldClass = FlowField;
        }
        field(44;"Sales LC/BG Utilised";Decimal)
        {
            CalcFormula = Sum("G/L Entry".Amount WHERE (System LC No.=FIELD(No.),
                                                        G/L Account No.=FILTER(300000..399999|205261)));
            Caption = 'Sales LC/BG Utilised';
            Editable = false;
            FieldClass = FlowField;
        }
        ...
        field(46;"Dealer Tenant ID";Code[30])
        {
            ...
            begin
                ...
                VALIDATE("Issued To/Received From",DealerInformation."Customer No.");
            end;
        }
        ...
        field(48;"Sales BG Utilised";Decimal)
        {
            CalcFormula = Sum("Detailed Cust. Ledg. Entry".Amount WHERE (Sys. LC No.=FIELD(No.)));
            Editable = false;
            FieldClass = FlowField;
        }
        field(49;"Remaining BG Value";Decimal)
        {
            Editable = false;
        }
        field(50;"Amended Value";Decimal)
        {
            CalcFormula = Sum("LC Amend. Details"."Amended Value" WHERE (No.=FIELD(No.)));
            Editable = false;
            FieldClass = FlowField;
        }
    }
    ...
    keys
    {
        ...
        {
            SumIndexFields = "LC\BG Value","Latest Amended Value";
        }
    ...
    fieldgroups
    {
        fieldgroup(DropDown;"No.","LC\DO No.",Description,"Issuing Bank","Issued To/Received From","Date of Issue")
        {
        }
    }
    ...
    begin
        IF "No." = '' THEN BEGIN
          LCSetup.GET;
          LCSetup.TESTFIELD("LC Details Nos.");
          NoSeriesMngt.InitSeries(LCSetup."LC Details Nos.",xRec."No. Series",0D,"No.","No. Series");
        END;
    ...
    begin
        WITH LCDetails DO BEGIN
          ...
          LCSetup.GET;
          LCSetup.TESTFIELD("LC Details Nos.");
          IF NoSeriesMngt.SelectSeries(LCSetup."LC Details Nos.",xRec."No. Series","No. Series") THEN BEGIN
            LCSetup.GET;
            LCSetup.TESTFIELD("LC Details Nos.");
