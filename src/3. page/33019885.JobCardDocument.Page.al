page 33019885 "Job Card Document"
{
    PageType = Document;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Manual Job Card No."; "Manual Job Card No.")
                {
                }
                field("Manual Job Card Date"; "Manual Job Card Date")
                {
                }
            }
                ...
                action("<Action1102159074>")
                {
                    ...
                    begin
                        ...
                        IF (MFG <> '') AND ("Battery Serial No." <> '') THEN BEGIN
                            ...
                            // to delete record from Batt-Service Job Card Table after posting
                            GblJobCardEntry.SETRANGE("Job Card No.", BatteryMaster."Job Card No.");
                            IF GblJobCardEntry.FIND('-') THEN
                                GblJobCardEntry.DELETE;

                            //to delete record from job card line table after posting
                            JobCardLine.SETRANGE("Job Card No.", "Job Card No.");
                            IF JobCardLine.FIND('-') THEN
                                REPEAT
                                    JobCardLine.DELETE;
                                UNTIL JobCardLine.NEXT = 0;
