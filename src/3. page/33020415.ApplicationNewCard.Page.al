page 33020415 "Application New Card"
{
    PageType = Card;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Employee Req Code"; "Employee Req Code")
                {
                }
            }
            ...
            group()
            {
                action("Select For Interview")
                {
                    ...
                    begin
                        ...
                        IF AppNew.FINDFIRST THEN BEGIN
                           IF AppNew.Interview = TRUE THEN BEGIN
                           MESSAGE(text0004);
                           END ELSE BEGIN
                           AppNew.Interview := TRUE;
                           AppNew."Posted by- Interview" := USERID;
                           AppNew."Posted Date- Interview" := TODAY;
                           AppNew.MODIFY;
                           MESSAGE(text0003);
                           END;
                        END;
                ...
                action(ShortList)
                {
                    ...
                    begin
                        ...
                        IF AppNew.FIND('-') THEN BEGIN
                            STPLSysMgt.SendEmailForShortlistApplicant(Rec.Email);
                            AppNew.ShortList := TRUE;
                            AppNew.Status := AppNew.Status::Shortlist;
                            AppNew."Written Exam" := TRUE;
                            AppNew."Posted by- Shortlist" := USERID;
                            AppNew."Posted Date- Shortlist" := TODAY;
                            AppNew.MODIFY;
                            MESSAGE(text0001);
                ...
                action("Convert To Interview")
                {
                    ...
                    begin
                        ...
                        Rec.TESTFIELD(Status, Rec.Status::Shortlist);
                ...
                action(Reject)
                {
                    ...
                    begin
                        ...
                        STPLSysMgt.SendEmailForRejectApplicant(Rec.Email);
                        Rec.Status := Rec.Status::Reject;
                        Rec."Posted by- Reject" := USERID;
                        Rec."Posted Date- Reject" := TODAY;
                        Rec.MODIFY;
                ...
                action(Writtern)
                {
                    ...
                    trigger OnAction()
                    begin
                        IF NOT CONFIRM('Do you want to convert this Application to Writtern?',FALSE) THEN
                          EXIT;

                        Rec.Status := Rec.Status::Writtern;
                        Rec.MODIFY;
    ...
    begin
        AppNew.SETRANGE(AppNew."Application No.","Application No.");
        IF AppNew.FIND('-') THEN BEGIN
           IF (AppNew.ShortList = TRUE)  THEN BEGIN
              ShowFastTab := TRUE;
              ShowAction := TRUE;
           END;

        IF (AppNew.ShortList = FALSE)  THEN BEGIN
              //ShowFastTab1 := TRUE;
           ShowAction1 := TRUE;
           END;
        END;

        IF Status = Status::Shortlist THEN
          ShortList := TRUE
        ELSE IF Status = Status::Interview THEN
         Interviewd := TRUE
        ELSE IF Status = Status::Reject THEN
          Reject := TRUE
        ELSE IF Status = Status::Writtern THEN
          Writtern := TRUE;
    end;

    var
        AppNew: Record "33020382";
        text0001: Label 'This Applicant is shortlisted.';
        text0002: Label 'Already ShortListed Applicant.';
        [InDataSet]
        ShowFastTab: Boolean;
        [InDataSet]
        ShowAction: Boolean;
        text0003: Label 'This Applicant is selected for Interview';
        text0004: Label 'Already Selected for Interview.';
        [InDataSet]
        ShowFastTab1: Boolean;
        [InDataSet]
        ShowAction1: Boolean;
        shortList: Boolean;
        Reject: Boolean;
