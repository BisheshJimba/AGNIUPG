page 25006184 "Service Order Subform EDMS"
{
    ...
    PageType = ListPart;
    Permissions = TableData 50005 = rim;
    ...
    layout
    {
        area(content)
        {
            repeater()
            {
                ...
                field(CalcOutboundTransferRes;CalcOutboundTransferRes)
                {
                ...
                field(Quantity;Quantity)
                {
                    ...
                    begin
                        ...
                        //IF FullyReservedToInventory = TRUE THEN
                         // ERROR(Text003);
                        *///Agni UPG 2009
                ...
                field("Approval Reason";"Approval Reason")
                {
                }
                field("Customer Verified";"Customer Verified")
                {
                    Visible = false;
                }
                ...
                field("Work Order Registered";"Work Order Registered")
                {
                    Visible = false;
                }
                ...
                field("External Service Purchased";"External Service Purchased")
                {
                    Editable = false;
                }
            }
        ...
        area(processing)
        {
            ...
            action("QR Specifications")
            {
                ...
                begin
                    ...
                    QRScan.SetServiceHeader(ServiceHeader,"Line No.");
                    QRScan.RUNMODAL;
                    CurrPage.UPDATE;
                end;
            }
            action("Create Lost Sales Entries")
            {
                ...
                begin
                    ...
                    IF NOT lostSalesEntries.FINDFIRST THEN
                      LostSalesMgt.CreateEntry_Item(ServiceHeader."Posting Date", Rec."No.",ServiceHeader."Sell-to Customer No.",'',Rec."Document No.",'',3,FALSE,FALSE,Rec.Quantity)
                    ELSE
    ...
    begin
        ...
        */
        IF Type = Type::Labor THEN BEGIN
          ...
          ServLaborAllocationApp.SETCURRENTKEY("Document Type","Document No.","Line No.");
          ServLaborAllocationApp.SETRANGE("Document Type",ServLaborAllocationApp."Document Type"::Order);
          ServLaborAllocationApp.SETRANGE("Document No.","Document No.");
          ServLaborAllocationApp.SETRANGE("Line No.","Line No.");
          ServLaborAllocationApp.SETRANGE("Time Line",TRUE);
          IF ServLaborAllocationApp.FINDFIRST THEN
            ERROR(AllocationExists,"Document No.","No.");
        END;
        IF (Type = Type::"External Service") AND "External Service Purchased" THEN
          ERROR('You cannot delete External Service Line unless you return corresponding Purchase Order.');
        //Agni 2009 UPG
    ...
    var
        ...
        Text002: Label 'You cannot delete Service line that are fully reserved to Inventory.';
        Text003: Label 'You cannot change Quantity on Service Line that are fully transfered from Inventory.';
        ServLaborAllocationApp: Record "25006277";
        AllocationExists: Label 'Schedule Entry Exists for Service Order %1, Labor No. %2. First Deallocate the Schedule & delete the line.';
    ...
    var
        ...
        ServiceHdrEDMS: Record "25006145";
        FilterTxt: Text[250];
        ServiceLineEDMS: Record "25006146";
        ReportServLineGatepass: Report "33020798";
    begin


        GatepassHeader.RESET;
        GatepassHeader.SETCURRENTKEY("External Document No.");
        GatepassHeader.SETRANGE("External Document No.","Document No.");
        ...
        BEGIN
          ...
          GatepassHeader."External Document No." := "Document No.";
          //GatepassHeader."Ship Address" := ServiceHdrEDMS.ship;
          GatepassHeader.VALIDATE("Issued Date",TODAY);
          GatepassHeader.Destination := 'OUT';
          GatepassHeader.INSERT(TRUE);
        END;

        ServiceLineEDMS.COPY(Rec);
        CurrPage.SETSELECTIONFILTER(ServiceLineEDMS);
        IF ServiceLineEDMS.FINDSET THEN REPEAT
          GatepassLine.RESET;
          GatepassLine.SETRANGE("Document Type",GatepassLine."Document Type"::"Vehicle Service");
          GatepassLine.SETRANGE("Ext Document No.", ServiceLineEDMS."Document No.");
          GatepassLine.SETRANGE("Item No.", ServiceLineEDMS."No.");
          IF NOT GatepassLine.FINDFIRST THEN BEGIN
              ...
              GatepassLine."Ext Document No." := ServiceLineEDMS."Document No.";
              GatepassLine.Quantity := ServiceLineEDMS.Quantity;
              GatepassLine."Item No." := ServiceLineEDMS."No.";
              GatepassLine."Item Description" := ServiceLineEDMS.Description;
              GatepassLine."Service Line Print" := TRUE;
